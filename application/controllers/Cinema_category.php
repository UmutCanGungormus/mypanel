...
class Cinema_category extends MY_Controller{
    ...
    public function save(){
        ...
        if($validate){
            ...
            $insert = $this->cinema_category_model->add(
                array(
            ...
            redirect(base_url("cinema_category"));
        } else {
            $viewData = new stdClass();
            $viewData->viewFolder = $this->viewFolder;
            $viewData->subViewFolder = "add";
    ...
    public function update($id){
        ...
        $seo_url=convertToSEO($this->input->post("title"));
        if($validate){
            $update = $this->cinema_category_model->update(
                array(
                    "id" => $id
                ), array(
                    "title" => $this->input->post("title")
            ...
            $viewData->subViewFolder = "update";
            $viewData->form_error = true;
            $viewData->item = $this->cinema_category_model->get(
                array(
                    "id"    => $id,
                )
            );
    ...
    public function delete($id){
        $delete = $this->cinema_category_model->delete(
            array(
                "id"    => $id
            )
        );
    ...
    public function isActiveSetter($id){
        if($id){
            ...
            $this->cinema_category_model->update(
                array(
                    "id"    => $id
                ),
                array(
